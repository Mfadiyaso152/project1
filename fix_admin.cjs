      ...
      <div className={\`p-6 rounded-3xl border shadow-xl flex flex-col gap-5 \${isLight ? 'bg-white border-slate-200' : 'bg-slate-900 border-slate-800'}\`}>
        <div className="flex flex-col gap-2">
          ...
          <div className="flex gap-2">
            <button onClick={() => setPlan('pro')} className={\`flex-1 py-3 rounded-xl font-bold border transition-colors \${plan === 'pro' ? 'bg-teal-500 text-white border-teal-500' : (isLight ? 'bg-slate-50 text-slate-500 border-slate-200' : 'bg-slate-950 text-slate-500 border-slate-800')}\`}>
              Pro (احترافي)
            </button>
      ...
      <div className="flex flex-col gap-4">
        {users.filter(u => u.role !== 'admin').map(user => (
          <div key={user.email} className={\`p-5 rounded-3xl border shadow-sm \${isLight ? 'bg-white border-slate-200' : 'bg-slate-900 border-slate-800'}\`}>
            ...
            <div className={\`p-3 rounded-xl text-sm \${isLight ? 'bg-slate-50 text-slate-700' : 'bg-slate-950 text-slate-300'}\`}>
...
`
fs.writeFileSync('components/AdminViews.tsx', content);
